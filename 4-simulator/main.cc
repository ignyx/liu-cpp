...
void print_help(char *arg0) {
  cerr << "usage: " << arg0 << " iterations lines step battery_voltage\n"
...
void parse_arguments(char **argv,
  ...
  try {
    ...
    cerr << "Failed to parse iterations : \"" << argv[1]
         << "\" is not an integer." << endl;
    exit(1);
  } catch (out_of_range const &) {
    cerr << "Failed to parse iterations : \"" << argv[1] << "\" is too big."
         << endl;
    exit(1);
  }
  ...
  try {
    ...
    cerr << "Failed to parse lines : \"" << argv[2] << "\" is not an integer."
         << endl;
    exit(1);
  } catch (out_of_range const &) {
    cerr << "Failed to parse lines : \"" << argv[2] << "\" is too big." << endl;
    exit(1);
  }
  ...
  try {
    ...
    cerr << "Failed to parse time_step : \"" << argv[3] << "\" is not a double."
         << endl;
    exit(1);
  } catch (out_of_range const &) {
    cerr << "Failed to parse time_step : \"" << argv[3] << "\" is too big."
         << endl;
    exit(1);
  }
  ...
  try {
    ...
    cerr << "Failed to parse battery_voltage : \"" << argv[4]
  ...
  if (iterations <= 0) {
    cerr << "Please enter a positive number of iterations." << endl;
    exit(1);
  } else if (lines <= 0) {
    cerr << "Please enter a positive number of displayed lines." << endl;
    exit(1);
  } else if (time_step <= 0) {
    cerr << "Please enter a positive time step." << endl;
    exit(1);
  } else if (battery_voltage <= 0) {
    cerr << "Please enter a positive battery_voltage." << endl;
    exit(1);
  } else if (lines > iterations) {
    cerr << "Please enter a number of iterations greater than the number of "
            "lines."
         << endl;
    exit(1);
  }
